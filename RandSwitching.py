	...
	while not(die_bitches):
		...
		############################################ PHASE 2.0 ############################################
			...
			if(len(grant_ports[id]) > 0):
				grant_ports[id] = [grant_ports[id][random.randint(0, len(grant_ports[id]) - 1)]]		
...
def output_Q_thread(id):
	...
	while not(die_bitches):
		...
		if(is_state(state.SCHED_1_MID)):
			...
			if(len(prop_ports[id]) is not 0):
				# Pick from random 
				prop_ports[id] = [prop_ports[id][random.randint(0, len(prop_ports[id]) - 1)]]
...
while len(transmitted_packets) < (num_packets * iNQs):
	...
	if(x < num_packets):
		for y in xrange(0, iNQs):
			...
			# while is_there:
			is_there = False
			cell_new = (random.randint(timer_tick + 1, timer_tick + 1), random.randint(0, oNQs - 1), y)
